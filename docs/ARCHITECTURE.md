...
- Nothing creates an `OutcomeRecord` at runtime — the outcome worker only writes revisions — and `measureOutcome` does not carry `diagnosisCategory` onto the record it builds, so the column stores null and category-based historical matching would find nothing if a writer existed — ledger R-32.
