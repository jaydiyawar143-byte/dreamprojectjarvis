...
| R-4 | 8 DB integration tests failing | **Medium** | Reproduced 2026-09-14 on a dedicated test database: 188 passed / 8 failed, the same 8 in every run — §8.2 | The outcome-record and budget-window behaviour those tests target has no passing test | Fix the tests, test code only: one recommendation per outcome test, `finalize()` where a finalized outcome is needed, fixture times relative to now | **CLASSIFIED** 2026-09-14 — 7 test bugs, 1 stale test, 0 product bugs. Fixes not started. **RUN** |
...
|---|---|---|---|---|---|
| P1-1 | Fix 8 DB integration tests | High | — | `@jarvis/db` green | TODO — classified 2026-09-14 (§8.2, R-4): 7 test bugs and 1 stale test; the fixes are test-only |
