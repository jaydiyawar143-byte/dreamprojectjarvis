...
| R-32 | `measureOutcome` never copies `diagnosisCategory` onto the record it builds | **High** | `packages/core/src/outcome-engine.ts:660` assembles `outcomeRecord` without the field, although `OutcomeRecordSchema` declares it, `PrismaOutcomeRepository.create` persists `record.diagnosisCategory ?? null`, and `findFinalizedOutcomes` filters on that column. Found 2026-09-16 while fixing the R-4 tests: the 11.8A query test passed the category as measurement input, the stored row was null, and the filtered query returned nothing | Category-based historical matching silently returns no matches. **No impact today:** no runtime path creates an outcome record — `OutcomeWorker` only writes revisions, and the only callers of `create` are tests | Carry the category through `measureOutcome`, then cover creation and category-filtered retrieval with a product-level test. Product change: not part of R-4, which is test-only | **RESOLVED** 2026-09-16 on local `main`, not committed. Part B (11.8A) carried `diagnosisCategory` through `measureOutcome` and covered creation plus category-filtered retrieval with a product-level test; runtime baseline creation now lives in `RecommendationExecutionService` (`packages/tools/src/recommendation-bridge.ts`). The second half of the rationale — the worker was never scheduled — is closed separately below |
| R-33 | The marketing-intelligence pipeline was script-only; no runtime entry point carried the runtime checks | **High** | Reads, diagnosis and recommendation generation lived behind `apps/api/scripts/phase116b/*`, with no authorization, safety caps, in-flight guarding or persistence in front of them and no dashboard or voice way to trigger them | A person could not ask "analyze the account"; a script path would not have been a safe production entry | One shared, ports-only `AnalysisGenerator` reached by BOTH the JARVIS `meta.analyze` tool and `POST /api/v1/analysis`, parity-tested — the IntegrationCommandService pattern | **RESOLVED** 2026-09-17 on local `main`, not committed. `AnalysisGenerator` (`packages/tools/src/analysis-generator.ts`) executes the existing read-only Meta tools through the shared `ToolExecutor`, aggregates daily performance, runs anomaly detection + existing diagnosis/recommendation engines, persists a PROPOSED recommendation, and drives it from the route (`apps/api/src/routes/analysis.ts`) and the tool (`packages/tools/src/tools/meta-analysis-tool.ts`). Read-only by construction; per-(user, account) in-flight guard; `dryRun` stops exactly before persistence; the targeted ad account is ALWAYS the server-configured `META_AD_ACCOUNT_ID` (client accountId never read); persistence failures and unexpected throws return fixed messages, never internals. **RUN** 2026-09-17: generator 18/18, tool 17/17, route + parity 22/22, sprint6 mirrors green; repo-wide typecheck, lint 18/18 and build 18/18 clean; full tools 774/774, api 1,250 passed / 8 skipped, agents 552/552 |
...
| 2026-09-16 | Phase 11.7B runtime — schedule the outcome worker (local `main`, not committed, after R-20) | `OutcomeWorker` was complete and tested (11.7A/11.7B) but nothing invoked it at runtime; give the closed learning loop its heartbeat | 6 files (2 new): `packages/config/src/index.ts` + `.env.example` (`JARVIS_OUTCOME_WORKER_INTERVAL_MS`, default 300000, 0 disables), `apps/api/src/config/env.ts`, `apps/api/src/services/outcome-worker-scheduler.ts` (new), `apps/api/src/index.ts` (wiring after listen), `apps/api/test/outcome-worker-scheduler.test.ts` (new) | New tests watched failing first: the fake-timer harness first hung 30s because `vi.advanceTimersByTimeAsync` awaits a pending in-flight sweep — the tests now join sweeps synchronously via the returned handle and use sync `advanceTimersByTime` while one is pending — and a mock whose *second* call returned a never-resolved promise left `stop()` waiting on it, so later calls resolve immediately. packages/config build/typecheck/lint/test 44/44; apps/api typecheck, lint, build clean; apps/api suite 1,228 passed / 8 skipped (6 new, all green including the real-worker FINALIZED e2e); packages/core 642 passed | R-32 resolved (its "not scheduled" half); capability matrix and ARCHITECTURE gaps closed | `typecheck:tests` still reports 60 pre-existing errors in other test files, none from the new one; not committed; the running API/DB/browser was not exercised; no migration, Docker or `.env` file change | Commit R-20 and this phase; then P0-3/P1-2 evidence, the R-16 owner actions |
| 2026-09-17 | Phase 11.10 — on-demand account analysis (local `main`, not committed, after the outcome-worker phase) | Promote the script-only marketing-intelligence pipeline into ONE shared production service reached by BOTH the `meta.analyze` JARVIS tool and `POST /api/v1/analysis` — the IntegrationCommandService pattern | 15 files (7 source, 2 test new): `packages/tools/src/analysis-generator.ts` (new), `packages/tools/src/tools/meta-analysis-tool.ts` (new), `packages/tools/src/index.ts`, `packages/tools/test/analysis-generator.test.ts` (new), `packages/tools/test/meta-analysis-tool.test.ts` (new), `apps/api/src/services/container.ts`, `apps/api/src/routes/analysis.ts` (new), `apps/api/src/index.ts`, `apps/api/test/analysis.test.ts` (new), `apps/api/test/analysis-parity.test.ts` (new), `packages/agents/src/agent-policy.ts`, `packages/agents/test/helpers/sprint6-harness.ts`, `apps/api/test/sprint6-agent-wiring.test.ts`, `apps/web/src/lib/api.ts`, `apps/web/src/app/opportunities/page.tsx` | Root cause found first: the evidence payload in `diagnosis-prompt.ts` is `JSON.stringify` of an ARRAY, so fake providers must parse `Array.isArray(parsed) ? parsed[0] : parsed` and echo the array element's `evidenceHash` (schema requires ≥ 16 chars) — a `"evh"` echo failed schema → DIAGNOSIS_UNAVAILABLE. Fixed PERSIST_FAILED leak (static detail, the store's `PG_UNAVAILABLE_INSIDE_STORE` never reaches outcomes) and the tool's unexpected-throw leak (static failure verb). generators/tool 18/18 + 17/17; route + parity 22/22; sprint6 mirrors green; repo typecheck, lint 18/18, build 18/18 clean; tools 774/774; api 1,250 passed / 8 skipped; agents 552/552 | R-33 resolved; recommendation generation now a user-reachable capability (matrix updated); docs updated (phase-11, matrix, SKILLS, API, ARCHITECTURE, ledger, diagram) | Not committed; the running API/DB/browser was not exercised; the `dryRun`/button path was verified only by route tests, not a real Meta account; no migration, Docker or `.env` change | Commit this phase; then P0-3/P1-2 evidence and the R-16 owner actions |
...
**Update 2026-09-16, after the outcome-worker scheduling phase:** the last 11.7B gap is closed on local `main`, not committed — `OutcomeWorker` now runs on an interval in the API process (`startOutcomeWorkerSweep`, `JARVIS_OUTCOME_WORKER_INTERVAL_MS`) after the server listens, never overlapping itself, draining cleanly at shutdown, with an immediate first sweep for stale-lease recovery. Proof: `apps/api/test/outcome-worker-scheduler.test.ts` 6/6, apps/api 1,228 passed / 8 skipped, packages/core 642 passed (the 10k-record SCALE contention test green in isolation and, this run, in the full suite). This resolves the "worker is not scheduled" half of **R-32**; its code half (the `diagnosisCategory` carry) was closed in Part B. Next: commit R-20 and this phase; then P1-2 evidence and the R-16 owner actions. The R-5 `phase102` flake remains unclassified.

**Update 2026-09-17, after Phase 11.10:** the marketing-intelligence pipeline is no longer script-only — **R-33** is closed with one shared `AnalysisGenerator` reached by the `meta.analyze` tool and `POST /api/v1/analysis`, and the Opportunities page now has an "Analyze account" button. Proof: generator 18/18, tool 17/17, route + parity 22/22; repo-wide typecheck, lint 18/18 and build 18/18 clean; full tools 774/774, api 1,250 passed / 8 skipped, agents 552/552. Next: commit the phase; then P0-3/P1-2 evidence and the R-16 owner actions. The R-5 `phase102` flake remains unclassified.
